function init(args)
    if not self.initialized and not args then
        ...
        if storage.position == nil then
            local pos = entity.position()
            storage.position = {pos[1] + (0.5), pos[2] + 1.8}
        end
    end
end
...
function main()
    if self.revive then
        if self.revive == 5 then
            ...
            local objectIds = world.npcQuery(storage.position, 1, {callScript = "isMannequin"})
...
function spawnMannequin()
    ...
    storage.mannequin.id = world.spawnNpc(storage.position, storage.mannequin.npcSpecies, "mannequin", 1, 0, storage.mannequin.npcParameter)
...
function die()
    if storage.mannequin.id then
       ...
       if pos then
           ...
           world.spawnProjectile("ele_invdamage", pos, id, { 0, 1 }, false, {power = 28, speed = 0.1, damageKind = "invisible" })
       end
    end
end
