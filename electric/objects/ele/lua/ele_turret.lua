...
function onInteraction(args)
    ...
    if storage.energy < 15 then
        setEnergy(15)
    end
    if isActive() and storage.energy ~= 15 then
        setActive(false)
    else
        setActive(true)
    end
...
function potentialTargets()
    ...
    local monsterIds = world.monsterQuery(getBasePosition(), radius, { notAnObject = true, validTargetOf = entity.id() })
...
function deadState.validate()
    if storage.energy > 0 and isActive() then
        return false
    end
...
function deadState.enteringState(stateData)
    ...
    if storage.energy < 0 then setEnergy(0) end
end

function deadState.update(dt, stateData)
    energyApi.checkSometimes(75)
    local rotationRange = entity.configParameter("rotationRange") * math.pi / 180;
    entity.rotateGroup("gun", -rotationRange)
...
function scanState.update(dt, stateData)
    ...
    setEnergy(storage.energy - (dt * energyTickTime))
...
function attackState.update(dt, stateData)
    ...
    if haveTarget then
        ...
        if stateData.fireTimer >= fireCooldown then
            ...
            world.spawnProjectile(bulletType, tipPosition, entity.id(), aimVector, false, entity.configParameter("projectileConfig"))
            stateData.fireTimer = 0

            local fireSound = entity.configParameter("fireSound")
            entity.playImmediateSound(fireSound)

            local energyPerShot = energyApi.usage
            setEnergy(energy-energyPerShot)
        end
